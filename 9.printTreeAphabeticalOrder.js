// Q9.
// From following tree https://drive.google.com/uc?id=1WiTdeqPgpQlgCBgSuwRGCcjajAO3JHNw&export=download
// Visit the nodes in alphabetical order and return an array of their values in that order.
// Look in the /assets folder of this repo to see a picture of the tree
// Another text visualization of the tree could be
//            (F)
//       (D)        (J)
//    (B)  (E)   (H)  (K)
//  (A) (C)    (G) (I)
// The tree is already built for you in the code below.

// class BinaryTreeNode {
//   constructor(val) {
//     this.val = val;
//     this.left = null;
//     this.right = null;
//   }
// }
// 
// f is the root, so therefore
// printTreeAlphabeticalOrder(f) Should return the array 
// [ "A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K" ]

// Implement your code here
const printTreeAlphabeticalOrder = (node, result = []) => {

}


module.exports = printTreeAlphabeticalOrder